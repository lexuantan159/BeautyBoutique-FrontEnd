    ...
    >
      <div className=" dark:bg-white  modal-box relative left-0 right-0 overflow-y-scroll no-scrollbar rounded-lg sm:max-w-full">
        ...
        <div className="flex gap-3 flex-col text-slate-500">
          ...
          <LabelInfo label="Product name" info={product?.name || ''} />
          <LabelInfo label="Product quantity" info={product?.quantity || ''} />
          <LabelInfo label="Actual price" info={product?.quantity || ''} />
          <LabelInfo label="Sale Price" info={product?.discountPercent || ''} />
          <LabelInfo label="Brand" info={product?.brand || ''} />
          <LabelInfo label="Category" info={product?.Category || ''} />
          <LabelInfo
            description={true}
            label="Description"
