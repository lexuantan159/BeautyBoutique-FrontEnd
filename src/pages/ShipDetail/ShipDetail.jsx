...
const ShipDetail = () => {
  ...
  const { notify, formatNumber, toastLoadingId, toastUpdateLoadingId } =
    useContext(MethodContext);
  const navigate = useNavigate();
  const { data: shipDetails, isLoading } = useQuery(['shipDetails'], () =>
    shipDetailService.getShipDetails({ userId: 1 })
  );
  const { data: cartData } = useQuery(['cart', action], () =>
  ...
  const handlePayment = async () => {
    ...
    if (methodPayment?.id === 1) {
      const responseCreateOrder = await orderServices.createOrder(
        paramsOrder,
        cartItem
      );
      if (responseCreateOrder.status === 201) {
        ...
        toastUpdateLoadingId(
          responseCreateOrder?.response?.data || 'Create order fail!',
          'error',
          id
        );
      }
    ...
    const responseCreatePayment = await orderServices.createPayment(
      paramsPayment,
      cartItem
    );
    ...
    <>
      <div className="max-w-[1200px] grid grid-cols-12 gap-5 mt-40 mx-10 lg:px-2 lg:mx-auto">
        ...
        <div className="bg-white col-span-12 lg:col-span-8  overflow-y-scroll no-scrollbar">
          <div className="p-3 mb-3 rounded-lg border-[0.2px] border-gray-300 shadow-lg">
            <p className="flex items-center text-lg font-bold">
              <MdOutlineAddLocationAlt className="mr-2" /> Address{' '}
            </p>
            <div className="grid grid-cols-2 gap-4 mt-3">
              {isLoading ? (
                <div>LOADING...</div>
              ) : shipDetails?.status === 200 &&
                shipDetails?.data?.shipDetails.length > 0 ? (
                shipDetails?.data?.shipDetails.slice(0, 3).map(item => {
                  return (
                    <div
                      key={item?.id}
                      onClick={() => setShipDetail(item)}
                      className={`p-3 border-[1px] hover:border-[#FF9FA0] rounded-lg hover:cursor-pointer ${
                        shipDetail?.id === item?.id && 'border-[#FF9FA0]'
                      } transition-all`}
                    >
            ...
            <ShipDetailItem
              shipDetails={shipDetails?.data?.shipDetails}
              itemShip={shipDetail}
          ...
          {/*Method*/}
          <DropList
            title={'method shipping'}
            listItem={[{ id: 2, name: 'J&T' }]}
            type={'shipping'}
            setItem={setMethodShipping}
          ></DropList>
          <DropList
            title={'method payment'}
            listItem={[
              { id: 1, name: 'Thanh Toan Khi Nhan Hang' },
              { id: 2, name: 'Zalo Pay' },
            ]}
            type={'payment'}
            setItem={setMethodPayment}
          ></DropList>
        </div>
        <div className="bg-white col-span-12 lg:col-span-4 flex-grow">
          ...
          <div className="w-full mt-4 bg-white rounded-lg shadow-md p-3 max-h-[260px] overflow-y-scroll no-scrollbar border-[0.2px] border-gray-300">
            {isLoading ? (
              <span className="loading loading-dots loading-lg text-xl "></span>
            ) : (
              cartData.status === 200 &&
              cartData?.data?.carts.length > 0 &&
              cartData?.data?.carts.map(item => {
                return (
                  <Link to="/">
                    <div
                      key={item?.product?.id}
          ...
          {/*Voucher*/}
          <VoucherCom
            conditionApply={totalPrice}
            voucherActive={setVoucher}
          ...
          <div className="w-full mt-4 bg-white rounded-lg shadow-md px-3 mb-2 border-[0.2px] border-gray-300">
            <div className="mx-3 mb-3 flex justify-between items-center border-b-[1px] border-gray-300">
              ...
              <p className="py-2 ">
                {formatNumber(cartData?.data?.totalPrice)}${' '}
              </p>
            </div>
            <div className="mx-3 mb-3 flex justify-between items-center border-b-[1px] border-gray-300">
